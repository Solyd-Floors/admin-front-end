...
class App extends React.Component {
    // as props now 
    // import this.props.getDeleteModelGQL from "../../gql/mutations/delete_videos.graphql";
    // import this.props.getUpdateModelGQL from "../../gql/mutations/update_videos.graphql";
    // const this.props.GET_MODELS = loader("../../gql/queries/get_videos.graphql")
    // const this.props.POST_MODELS = loader("../../gql/mutations/post_videos.graphql")
    // this.props.columns
    // this.props.text.title
    // this.props.text.loading

    // this.props.getModelKey
    // this.props.getModelSubKey
    ...
    updateData = async () => {
        try {
            let cache = await this.props.client.readQuery({
                query: this.props.GET_MODELS
            })
            this.setData(cache[this.props.getModelField].data.videos)
        } catch (err) { }
    }
    ...
    handleRowAdd = async (newData, resolve, reject) => {
        ...
        try {
            let val = await this.props.modelSchema.validate(args, { abortEarly: false });
        } catch (err) {
        ...
        let success = await this.handleMutate(() => {
            return this.props.client.mutate({
                mutation: this.props.POST_MODELS,
                variables: args,
                refetchQueries: [{ query: this.props.GET_MODELS }]
            })
        })
    ...
    handleRowDelete = async (oldData, resolve, reject) => {
        ...
        let success = await this.handleMutate(() => {
            return this.props.client.mutate({
                mutation: this.props.getDeleteModelGQL(oldData.id)
            })
        })
        ...
        cache[this.props.getModelField].data.videos = cache[this.props.getModelField].data.videos.filter(x => x.id != oldData.id)
    ...
    handleRowUpdate = async (newData, oldData, resolve,reject) => {
        ...
        let success = await this.handleMutate(() => {
            return this.props.client.mutate({
                mutation: this.props.getUpdateModelGQL(oldData.id),
                variables: args
            })
        })
        return success ? resolve() : reject();
    ...
    render() {
        return (
            <React.Fragment>
                <Query query={this.props.GET_MODELS} fetchPolicy={"cache-and-network"}>
                            ...
                            <Grid spacing={1}>
                                ...
                                <Grid item xs={12}>
                                    ...
                                    <MaterialTable
                                        ...
                                        title={`${this.props.text.title} ${loading && !data ? `( ${this.props.text.loading} )` : ""}`}
                                        ...
                                        data={
                                            ...
                                            ? 
                                                [] 
                                            : 
                                                data[this.props.getModelKey].data[this.props.getModelSubKey]
                                        }
                                        icons={tableIcons}
                                        editable={{
