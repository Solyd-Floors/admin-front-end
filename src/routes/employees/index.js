...
import getDeleteCountryGQL from "../../gql/mutations/delete_employees.graphql";
import getUpdateCampaignCategoryGQL from "../../gql/mutations/update_employees.graphql";
...
const GET_BRANDS = loader("../../gql/queries/get_employees.graphql")
const POST_BRANDS = loader("../../gql/mutations/post_employees.graphql")
...
class App extends React.Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            { title: "password", field: "password", render: t => <p>*******</p>, hidden: false },
            { title: "full_name", field: "full_name", hidden: false },
            { title: "BusinessId", field: "BusinessId", type: "numeric", hidden: false },
        ]
    ...
    updateData = async () => {
        try {
            ...
            this.setData(cache.getEmployees.data.employees)
        } catch (err) { }
    }
    ...
    handleRowDelete = async (oldData, resolve, reject) => {
        ...
        cache.getEmployees.data.employees = cache.getEmployees.data.employees.filter(x => x.id != oldData.id)
            ...
            <React.Fragment>
                <Query query={GET_BRANDS} fetchPolicy={"cache-and-network"}>
                    {({ loading, error, data, refetch }) => {
                        ...
                        console.log({ data: loading && !data ? [] : data.getEmployees.data.employees})
                        return (
                            <Grid spacing={1}>
                                <Grid item xs={3}></Grid>
                                <Grid item xs={12}>
                                    ...
                                    <MaterialTable
                                        ...
                                        title={`Employees ${loading && !data ? "( loading )" : ""}`}
                                        columns={this.columns}
                                        data={loading && !data ? [] : data.getEmployees.data.employees}
                                        icons={tableIcons}
                                        editable={{
