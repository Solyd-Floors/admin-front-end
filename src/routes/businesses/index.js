...
import getDeleteCountryGQL from "../../gql/mutations/delete_businesses.graphql";
import getUpdateCampaignCategoryGQL from "../../gql/mutations/update_business.graphql";
...
const GET_BRANDS = loader("../../gql/queries/get_businesses.graphql")
const POST_BRANDS = loader("../../gql/mutations/post_businesses.graphql")
...
class App extends React.Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            { title: "phone_number", field: "phone_number", hidden: false },
            { title: "UserId", field: "UserId", type: "numeric", hidden: false },
            { title: "IndustryId", field: "IndustryId", type: "numeric", hidden: false },
        ]
    ...
    updateData = async () => {
        try {
            ...
            this.setData(cache.getBusinesses.data.businesses)
        } catch (err) { }
    }
    ...
    handleRowDelete = async (oldData, resolve, reject) => {
        ...
        cache.getBusinesses.data.businesses = cache.getBusinesses.data.businesses.filter(x => x.id != oldData.id)
            ...
            <React.Fragment>
                <Query query={GET_BRANDS} fetchPolicy={"cache-and-network"}>
                    {({ loading, error, data, refetch }) => {
                        ...
                        console.log({ data: loading && !data ? [] : data.getBusinesses.data.businesses})
                        return (
                            <Grid spacing={1}>
                                <Grid item xs={3}></Grid>
                                <Grid item xs={12}>
                                    ...
                                    <MaterialTable
                                        ...
                                        title={`Businesses ${loading && !data ? "( loading )" : ""}`}
                                        columns={this.columns}
                                        data={loading && !data ? [] : data.getBusinesses.data.businesses}
                                        icons={tableIcons}
                                        editable={{
