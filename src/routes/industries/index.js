...
import getDeleteCountryGQL from "../../gql/mutations/delete_industries.graphql";
import getUpdateCampaignCategoryGQL from "../../gql/mutations/update_industry.graphql";
...
const GET_BRANDS = loader("../../gql/queries/get_industries.graphql")
const POST_BRANDS = loader("../../gql/mutations/post_industries.graphql")
...
class App extends React.Component {
    ...
    updateData = async () => {
        try {
            ...
            this.setData(cache.getIndustries.data.industries)
        } catch (err) { }
    }
    ...
    handleRowDelete = async (oldData, resolve, reject) => {
        ...
        cache.getIndustries.data.industries = cache.getIndustries.data.industries.filter(x => x.id != oldData.id)
            ...
            <React.Fragment>
                <Query query={GET_BRANDS} fetchPolicy={"cache-and-network"}>
                    {({ loading, error, data, refetch }) => {
                        ...
                        console.log({ data: loading && !data ? [] : data.getIndustries.data.industries})
                        return (
                            <Grid spacing={1}>
                                <Grid item xs={3}></Grid>
                                <Grid item xs={12}>
                                    ...
                                    <MaterialTable
                                        ...
                                        title={`Industries ${loading && !data ? "( loading )" : ""}`}
                                        columns={this.columns}
                                        data={loading && !data ? [] : data.getIndustries.data.industries}
                                        icons={tableIcons}
                                        editable={{
