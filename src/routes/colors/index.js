...
import getDeleteCountryGQL from "../../gql/mutations/delete_colors.graphql";
import getUpdateCampaignCategoryGQL from "../../gql/mutations/update_color.graphql";
...
const GET_BRANDS = loader("../../gql/queries/get_colors.graphql")
const POST_BRANDS = loader("../../gql/mutations/post_colors.graphql")
...
class App extends React.Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            { title: "Color", field: "color", render: t => (
                <div style={{
    ...
    updateColor = color => color && this.setState({ color })
    ...
    updateData = async () => {
        try {
            ...
            this.setData(cache.getColors.data.colors)
        } catch (err) { }
    }
    ...
    handleRowDelete = async (oldData, resolve, reject) => {
        ...
        cache.getColors.data.colors = cache.getColors.data.colors.filter(x => x.id != oldData.id)
            ...
            <React.Fragment>
                <Query query={GET_BRANDS} fetchPolicy={"cache-and-network"}>
                    {({ loading, error, data, refetch }) => {
                        ...
                        console.log({ data: loading && !data ? [] : data.getColors.data.colors})
                        return (
                            <Grid spacing={1}>
                                <Grid item xs={3}></Grid>
                                <Grid item xs={12}>
                                    ...
                                    <MaterialTable
                                        ...
                                        title={`Colors ${loading && !data ? "( loading )" : ""}`}
                                        columns={this.columns}
                                        data={loading && !data ? [] : data.getColors.data.colors}
                                        icons={tableIcons}
                                        editable={{
                                            onRowUpdate: (newData, oldData) =>
                                                new Promise((resolve,reject) => {
                                                    this.handleRowUpdate({...newData, color: this.state.color || newData.color }, oldData, resolve, reject);

                                                }),
                                            onRowAdd: (newData) =>
                                                new Promise((resolve, reject) => {
                                                    this.handleRowAdd({...newData, color: this.state.color || newData.color }, resolve, reject)
                                                }),
                                            onRowDelete: (oldData) =>
                                                new Promise((resolve, reject) => {
