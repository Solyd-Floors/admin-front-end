...
import getDeleteCountryGQL from "../../gql/mutations/delete_floor_tile_size.graphql";
...
const GET_BRANDS = loader("../../gql/queries/get_floor_tile_sizes.graphql")
const POST_BRANDS = loader("../../gql/mutations/post_floor_tile_sizes.graphql")
...
class App extends React.Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            { title: "width", type: "numeric", field: "width", hidden: false },
            { title: "height", type: "numeric", field: "height", hidden: false },
        ]
    ...
    updateData = async () => {
        try {
            ...
            this.setData(cache.getFloorTileSizes.data.floor_tile_sizes)
        } catch (err) { }
    }
    ...
    handleRowDelete = async (oldData, resolve, reject) => {
        ...
        cache.getFloorTileSizes.data.floor_tile_sizes = cache.getFloorTileSizes.data.floor_tile_sizes.filter(x => x.id != oldData.id)
            ...
            <React.Fragment>
                <Query query={GET_BRANDS} fetchPolicy={"cache-and-network"}>
                    {({ loading, error, data, refetch }) => {
                        ...
                        console.log({ data: loading && !data ? [] : data.getFloorTileSizes.data.floor_tile_sizes})
                        return (
                            <Grid spacing={1}>
                                <Grid item xs={3}></Grid>
                                <Grid item xs={12}>
                                    ...
                                    <MaterialTable
                                        ...
                                        title={`Floor Tile Sizes ${loading && !data ? "( loading )" : ""}`}
                                        columns={this.columns}
                                        data={loading && !data ? [] : data.getFloorTileSizes.data.floor_tile_sizes}
                                        icons={tableIcons}
                                        editable={{
                                            onRowUpdate: false,
